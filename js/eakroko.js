    ...
    if ($('.dataTable').length > 0) {
        $('.dataTable').each(function () {
            if (!$(this).hasClass("dataTable-custom")) {
                ...
                var oTable;
                if ($(this).data('serialNumber') !== undefined) {
                    var snColumn = parseInt($(this).data('serialNumber'), 10);
                    if (snColumn > -1) {
                        opt.fnRowCallback = function (nRow, aData, iDisplayIndex) {
                            setTimeout(function () {
                                var oSettings = oTable.fnSettings();
                                $($("td", nRow).get(snColumn)).html(oSettings._iDisplayStart + iDisplayIndex + 1);
                                return nRow;
                            }, 10);
                        };
                    }
